...
- 주석은 `#` 사용
- 메서드별 독립 블럭
- `sources` 헤더: 문서가 설명하는 소스 파일과 작성 시점의 content hash.
  본문은 사람이 쓴다. 도구는 hash 불일치로 stale 여부만 알린다.
  본문 갱신 후 `lnt doc --stamp modulename`으로 hash를 다시 기록한다
